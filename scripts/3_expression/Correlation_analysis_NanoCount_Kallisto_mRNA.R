...
resdir="results/Nanocount_Illumina_tpm"
dir.create(resdir)
...
#df = data.frame(strsplit(as.character(gc$Name), "\\|"))
#gc1 = t(df)
#type_index = 8
#levmatr = makeAsLevels(gc1[,type_index], nme = "", join ="")
#levs = names(levmatr)
...
all_data_norm_HT = readAllDataNanoCount(data_dir, samples_HT, types = NULL, cnt_type="tpm",incl = TRUE, gc= gc) ##########################
.dim1<-function(f) dim(f$exons)
.dim2<-function(f) dim(f$gc)

#remove null exons(due to just one entry in category)
nullexons = which(unlist(lapply(lapply(all_data_norm_HT, .dim1),is.null)))
if(length(nullexons)>0){
  all_data_norm_HT = all_data_norm_HT[-nullexons]
}
lapply(all_data_norm_HT, .dim2)

all_data_norm_HT_ = all_data_norm_HT
mi = unlist(lapply(all_data_norm_HT_, function(f) dim(f$exons)[2]))
all_data_norm_HT_ = all_data_norm_HT_[mi==max(mi)]


for(i in 1:length(all_data_norm_HT)){
  all_data_norm_HT_[[i]]$exons =  cbind(all_data_norm_HT[[i]]$exons,all_data_norm_HT[[i]]$gc$X.GC)
}
all_data_norm_HT__ = all_data_norm_HT_

params = fromJSON(paste(rapidshome,"biotypes.json", sep="/"), simplifyDataFrame  = F)
for(i in 1:length(params)){
  print(params[[i]])
  all_data_norm_HT__  = mergeTypes1(all_data_norm_HT__, params[[i]], names(params)[[i]])
}

all_data_norm_HT2 = lapply(all_data_norm_HT__, mergeSameName)
...
for(i in 1:length(all_data_norm_K2))  hybrid_[[i]]  =  mergeNanoKallisto(all_data_norm_K2[[i]], all_data_norm_HT2[[i]], replace=T,datatype=names(all_data_norm_K2)[[i]]) ####################

for(i in 1:length(all_data_norm_K2))  hybrid[[i]]  =  mergeNanoKallisto(all_data_norm_K2[[i]], all_data_norm_HT2[[i]], replace=T,datatype=names(all_data_norm_K2)[[i]])

for(i in 1:length(all_data_norm_K_)) {
  hybrid3[[i]]  = mergeNanoKallisto(all_data_norm_K_[[i]], all_data_norm_HT_[[i]], replace=T,datatype=names(all_data_norm_K_)[[i]])
...
#hybrid = normaliseH(hybrid, sum_all_types=T) #what does normaliseH do? Each datatype add a "data_high" and "data_low"
#hybrid_ = normaliseH(hybrid)
#hybrid3 = normaliseH(hybrid3)
hybrid =removeGlobin(hybrid, annotate=F) ##remove Globin genes
hybrid_ =removeGlobin(hybrid_, annotate=F)
hybrid3=removeGlobin(hybrid3, annotate=F)

hybrid=hybrid[-length(hybrid)]
hybrid_=hybrid_[-length(hybrid_)]
hybrid3=hybrid3[-length(hybrid3)]
################### correlations on sample to sample level ##################
h = mergeAll(hybrid) #, "S"))
##h = normaliseH_(h)
...
saveRDS(res_all_hybrid,resdir%&%"/res_all_hybrid.rds")
#saveRDS(hybrid,resdir%&%"/hybrid.rds")
...
df_pcorr_full=read.table(paste(resdir,"/mRNA/corr_genes_50.txt",sep=""))
...
comps_des=c("6201_old_vs6679_old_")
...
plotDensity(h,list(25:26, 27:28), rev(lev), rm = torm)
plotDensity(h,list(29:30, 31:32), rev(lev), rm = torm)
dev.off()
